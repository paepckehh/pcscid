...
**One short, stable ID for every smart card you present — any card, any reader, pure Go.**
...
The same card always yields the same ID, on every reader, on every presentation. Two cards of the same model yield **different** IDs, because the identifier is derived from the card's own unique tag, not just its type.
...
3. The ATR yields the card type; type plus UID (or ATR) yields the ID.
...
```text
cmd/pcscid/       sample app: bare IDs in normal mode, full trace with DEBUG=1
